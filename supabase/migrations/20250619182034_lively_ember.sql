...
-- 删除可能存在的表和策略
DROP POLICY IF EXISTS "用户只能管理自己的跟踪链接" ON ip_tracking_links;
DROP POLICY IF EXISTS "链接创建者可以查看访问记录" ON visitor_ip_logs;
DROP POLICY IF EXISTS "允许匿名访问记录" ON visitor_ip_logs;
DROP POLICY IF EXISTS "链接创建者可以查看会话" ON visitor_sessions;
DROP POLICY IF EXISTS "允许匿名会话记录" ON visitor_sessions;
DROP POLICY IF EXISTS "链接创建者可以查看异常检测" ON ip_anomaly_detection;
DROP POLICY IF EXISTS "允许系统插入异常检测结果" ON ip_anomaly_detection;

DROP TRIGGER IF EXISTS trigger_detect_ip_anomalies ON visitor_ip_logs;
DROP TRIGGER IF EXISTS trigger_update_visitor_session ON visitor_ip_logs;
DROP TRIGGER IF EXISTS update_ip_tracking_links_updated_at ON ip_tracking_links;
DROP TRIGGER IF EXISTS update_visitor_sessions_updated_at ON visitor_sessions;

DROP FUNCTION IF EXISTS detect_ip_anomalies();
DROP FUNCTION IF EXISTS update_visitor_session();

DROP TABLE IF EXISTS ip_anomaly_detection CASCADE;
DROP TABLE IF EXISTS visitor_sessions CASCADE;
DROP TABLE IF EXISTS visitor_ip_logs CASCADE;
DROP TABLE IF EXISTS ip_tracking_links CASCADE;

-- IP跟踪链接表
CREATE TABLE ip_tracking_links (
...
-- IP异常检测结果表
CREATE TABLE ip_anomaly_detection (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- 创建索引
CREATE INDEX idx_ip_tracking_links_code ON ip_tracking_links(link_code);
CREATE INDEX idx_ip_tracking_links_user_id ON ip_tracking_links(user_id);
CREATE INDEX idx_ip_tracking_links_active ON ip_tracking_links(is_active) WHERE is_active = true;

CREATE INDEX idx_visitor_ip_logs_tracking_link ON visitor_ip_logs(tracking_link_id);
CREATE INDEX idx_visitor_ip_logs_ip ON visitor_ip_logs(ip_address);
CREATE INDEX idx_visitor_ip_logs_created_at ON visitor_ip_logs(created_at DESC);
CREATE INDEX idx_visitor_ip_logs_suspicious ON visitor_ip_logs(is_suspicious) WHERE is_suspicious = true;

CREATE INDEX idx_visitor_sessions_tracking_link ON visitor_sessions(tracking_link_id);
...
CREATE INDEX idx_visitor_sessions_suspicious ON visitor_sessions(is_suspicious) WHERE is_suspicious = true;

CREATE INDEX idx_anomaly_detection_tracking_link ON ip_anomaly_detection(tracking_link_id);
CREATE INDEX idx_anomaly_detection_type ON ip_anomaly_detection(anomaly_type);
CREATE INDEX idx_anomaly_detection_severity ON ip_anomaly_detection(severity);
CREATE INDEX idx_anomaly_detection_created_at ON ip_anomaly_detection(created_at DESC);
